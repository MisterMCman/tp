...
export default function MessagesInterface({
  ...
  // Helper to get the related data (works with both inquiry and trainingRequest)
  const getRelatedData = (message: MessageItem) => {
    const related = message.inquiry || message.trainingRequest;
    if (!related) return null;
    return {
      id: related.id,
      training: related.training,
      trainer: related.trainer
    };
  };
  ...
  return (
    <div className="bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen p-6">
      <div className="max-w-7xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredMessages.map((message) => {
                const related = getRelatedData(message);
                if (!related) return null; // Skip if no related data
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-600 mb-1">
                            Von: {related.trainer.firstName} {related.trainer.lastName}
                            ({related.trainer.email})
                          </p>
                          <p className="text-sm text-slate-500">
                            Training: {related.training.title} • {related.training.location}
                          </p>
                        </div>
        ...
        {selectedMessage && (() => {
          const related = getRelatedData(selectedMessage);
          if (!related) return null;
                  ...
                  <div className="bg-slate-50 p-4 rounded-lg mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{related.training.title}</p>
                      </div>
                      <div>
                        <span className="text-slate-500">Datum:</span>
                        <p className="font-medium">
                          {format(new Date(related.training.startDate), "dd.MM.yyyy", { locale: de })}
                        </p>
                      </div>
                      <div>
                        <span className="text-slate-500">Ort:</span>
                        <p className="font-medium">{related.training.location}</p>
                      </div>
                    </div>
                    <div className="mt-3 pt-3 border-t border-slate-200">
                      <Link
                        href={`/dashboard/training/${selectedMessage.trainingRequestId || selectedMessage.inquiryId}?from=${encodeURIComponent(pathname)}`}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-lg font-medium text-slate-800 mt-1">
                          {related.trainer.firstName} {related.trainer.lastName}
                        </p>
                        <p className="text-sm text-slate-600">{related.trainer.email}</p>
                      </div>
                      <Link
                        href={`/dashboard/trainer/${related.trainer.id}`}
